...
Each unit and tower costs income, and when the income is negative, the state reserve begins to deplete. When the money reaches 0, all units will die, freeing the income consumed.
...
To run the game, open the `src/` directory and type `python main.py`. Python version 3.*, as well as pygame, are expected to be installed in order to run the game.
...
To undo/redo an action, press `CTRL + z`/`CTRL + y` respectively.

To save/load a map (inside the editor), press `CTRL + s`/`CTRL + l` respectively.
...
* pygame: https://www.pygame.org/news A wrapper over SDL 2.0 for handling rendering, audio, input and events. The core essentials of a game. Used almost everywhere. 
...
* Ursescu Sebastian: Development of game logic and rendering, menus and map editor
* Stoian Andrei-Alexandru: Button systems, color management, and early architecture
...
To solve this, a main Button class was implemented, alongside child implementations of the button. Standard OOP philosophy.
...
and drawn on demand. Similar to the minecraft chunks system, only that in this project is used only for rendering.

# Afterthoughts

Although a really fun project to work on, we greatly underestimated the time and effort it took to finalize this project. Still, this proved to be a good exercise in implementing a real python application. 
What we tried to achieve is a lightweight, and modular game. We partly achieved this, the overlooked areas being serialized game parameters (that could be stored inside a .yaml file for example),
and the button system is quite limited in the tasks that it can do. Still, there are also great aspects, such as the game renderer, that despite the several optimizations lacking for a smoother experience,
can theoretically handle and render infinite maps, due to the rendering of chunks only visible to the screen. Definitely, a part of the project with the most work put into it.
...
Last but not least, this was designed as a team project, integrating work onto platforms like github. This project also highlighted the importance of consistent communication and shared commitment in team projects.
